<template>
  <div class="play-wrap">
    <div class="play-con" @click="playFn">
      <div class="logo">
      ...
      <div class="song-turn">
        <div class="play-turn">
          <div class="img-box" ref="img">
            <img :src="songImg" alt />
          </div>
          
          <div class="play-btn" v-show="isShow"></div>
        </div>
        <div class="top-turn"></div>
      </div>
      ...
      <div class="song-info">
        <div class="song-con">
          ...
          <span class="gap">-</span>
          <span class="autr">{{songs}}</span>
        </div>
        <audio :src="songUrl" ref="audio"></audio>
        <div ref="geciCont" class="ci">
          <p v-for="(v,k) in geci" :key="k" class="animated pulse">{{v}}</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    this.axios.get("http://localhost:3000/song/detail?ids=" + id).then(res => {
      ...
      this.alia = res.data.songs[0].alia[0];
      this.songs = res.data.songs[0].ar[0].name;
    });
    // 歌曲的歌词
    this.axios.get("http://localhost:3000/lyric?id=" + id).then(res => {
      ...
      let lyricObj = {};
      this.lyric = this.lyric.map(item => {
        item.match(regex);
        let key = RegExp.$1;

        let m = key.split(":")[0];
        let s = key.split(":")[1];
        key = parseInt(m) * 60 + parseFloat(s);
  ...
  methods: {
    ...
    addEvent() {
      this.$refs.audio.addEventListener("timeupdate", res => {
        // console.log(this.$refs.audio.currentTime);
        let ct = this.$refs.audio.currentTime.toFixed(3);
        this.count = ct;
        console.log(this.lyricObj);
        
        for (let k in this.lyricObj) {
          if (parseFloat(ct) >= parseFloat(k)) {
            this.geci[k] = this.lyricObj[k];
            this.$refs.geciCont.scrollTop = this.$refs.geciCont.scrollHeight;
          }
        }
      });
    }
...
.play-con {
  ...
  /* background: url(../assets/images/play-bg.jpg) 50% no-repeat; */
...
.play-turn {
  width: 100%;
  height: 100%;
  background: url(../assets/images/song-turn.png) no-repeat;
...
.top-turn {
  ...
  top: -0.8rem;
  left: 1.57rem;
  background: url(../assets/images/needle-plus.png) no-repeat;
  background-size: contain;
  z-index: 10;
}
.img-box {
